...
def lrt_free_psi_equal_variance(gene_name, abkt, y_ce1, y_ce0, theta_sigma, group_status, num_random_restarts, minrr):

    theta_g1 = theta_sigma[0]
    theta_g2 = theta_sigma[2]
    sigma_g1 = theta_sigma[1]
    sigma_g2 = theta_sigma[3]

    print theta_g1, theta_g2, sigma_g1, sigma_g2

    psi_lower, psi_upper = get_psi_range(y_ce1, y_ce0)
    
    real_params_g_rtimes = column_stack((logit(uniform(psi_lower, psi_upper, num_random_restarts)),
                                        log(uniform(sigma_g1, sigma_g1, num_random_restarts)), log(uniform(sigma_g2,sigma_g2,num_random_restarts))))
    arg_min_x=[]
    val_min_x=[]
    ...
    for i in range(num_random_restarts):
        ...
        real_params_g=real_params_g_rtimes[i,:]
        optim_result_obj=minimize(likelihoodumi.neg_log_sum_marginal_likelihood_psi_equal_variance, x0=real_params_g, args=(abkt, y_ce1, y_ce0, group_info, theta_g1, theta_g2, sigma_g1, sigma_g2, group_status), method='L-BFGS-B')
        if optim_result_obj.success and (not np.isnan(optim_result_obj.fun)) and (optim_result_obj.fun != 0):
            arg_min_x.append(optim_result_obj)
            val_min_x.append(optim_result_obj.fun)
        if len(arg_min_x) >= minrr:
    ...
    psi_lower0, psi_upper0, psi_lower1, psi_upper1 = get_psi_range_grp(y_ce1, y_ce0, group_info)
    
    real_params_g_rtimes = column_stack((logit(uniform(psi_lower0, psi_upper0, num_random_restarts)), logit(uniform(psi_lower1, psi_upper1, num_random_restarts)),
                                        log(uniform(sigma_g1, sigma_g1, num_random_restarts)),
                                        log(uniform(sigma_g2,sigma_g2,num_random_restarts))))
    arg_min_x=[]
    val_min_x=[]
    for i in range(num_random_restarts):
        log_fh.log('tasc free theta optimization #' + str(i) + ' for gene ' + gene_name)
        real_params_g=real_params_g_rtimes[i,:]
        optim_result_obj=minimize(likelihoodumi.neg_log_sum_marginal_likelihood_psi_free_equal_variance, x0=real_params_g, args=(abkt, y_ce1, y_ce0, group_info, theta_g1, theta_g2, sigma_g1, sigma_g2, group_status), method='L-BFGS-B')
        if optim_result_obj.success and (not np.isnan(optim_result_obj.fun)) and (optim_result_obj.fun != 0):
            arg_min_x.append(optim_result_obj)
            val_min_x.append(optim_result_obj.fun)
        if len(arg_min_x) >= minrr:
    ...
    if len(arg_min_x) == 0:
        res_fh.log((gene_name, False, float('nan'), float('nan'), float('nan'), float('nan'),
                    float('nan'), float('nan'), float('nan'), float('nan'), float('nan')))
        return
    tasc_free_psi = arg_min_x[np.argmin(val_min_x)]

    lrt_stat = 2 * (tasc_b_res.fun - tasc_free_psi.fun)
    if np.isnan(lrt_stat):
        res_fh.log((gene_name, False, float('nan'), float('nan'), float('nan'), float('nan'),
                    float('nan'), float('nan'), float('nan'), float('nan'), float('nan')))
    else:
        lrt_pval = 1 - chi2.cdf(lrt_stat, df=1)
        res_fh.log((gene_name, True, expit(tasc_b_res.x[0]), exp(tasc_b_res.x[1]), exp(tasc_b_res.x[2]),
                    expit(tasc_free_psi.x[0]), expit(tasc_free_psi.x[1]), exp(tasc_free_psi.x[2]),
                    exp(tasc_free_psi.x[3]), lrt_stat, lrt_pval))
    


def lrt_free_psi_equal_variance_umi(gene_name, abkt, y_ce1, y_ce0, theta_sigma, group_status, num_random_restarts, minrr):

    theta_g1 = theta_sigma[0]
    theta_g2 = theta_sigma[2]
    sigma_g1 = theta_sigma[1]
    sigma_g2 = theta_sigma[3]
    p_bursting = theta_sigma[4]

    print theta_g1, theta_g2, sigma_g1, sigma_g2, p_bursting
    ...
    for i in range(num_random_restarts):
        ...
        real_params_g=real_params_g_rtimes[i,:]
        optim_result_obj=minimize(likelihoodumi.neg_log_sum_marginal_likelihood_psi_equal_variance_umi, x0=real_params_g, args=(abkt, y_ce1, y_ce0, group_info, theta_g1, theta_g2, sigma_g1, sigma_g2, p_bursting, group_status), method='L-BFGS-B')
        if optim_result_obj.success and (not np.isnan(optim_result_obj.fun)) and (optim_result_obj.fun != 0):
            arg_min_x.append(optim_result_obj)
            val_min_x.append(optim_result_obj.fun)
        if len(arg_min_x) >= minrr:
    ...
    for i in range(num_random_restarts):
        log_fh.log('tasc free theta optimization #' + str(i) + ' for gene ' + gene_name)
        real_params_g=real_params_g_rtimes[i,:]
        optim_result_obj=minimize(likelihoodumi.neg_log_sum_marginal_likelihood_psi_free_equal_variance_umi, x0=real_params_g, args=(abkt, y_ce1, y_ce0, group_info, theta_g1, theta_g2, sigma_g1, sigma_g2, p_bursting, group_status), method='L-BFGS-B')
        if optim_result_obj.success and (not np.isnan(optim_result_obj.fun)) and (optim_result_obj.fun != 0):
            arg_min_x.append(optim_result_obj)
            val_min_x.append(optim_result_obj.fun)
        if len(arg_min_x) >= minrr:
